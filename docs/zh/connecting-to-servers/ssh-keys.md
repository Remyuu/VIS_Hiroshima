...
=== "Linux"
    ...
    大多数 Linux 发行版已经预装 OpenSSH 客户端。推荐使用系统自带的终端，或者你喜欢的话也可以使用 Ghostty。
    ...
    如果屏幕上显示 SSH 的版本信息，就说明 SSH 已经可以正常使用。

    如果系统提示找不到 `ssh` 命令，可以通过发行版自带的包管理器安装。例如 Ubuntu / Debian 可以使用：
...
如果你使用的是管理员提供的初始密码，例如“今日日期”这类临时密码，第一次登录成功后请立即重设成只有自己知道的密码。
...
和 SSH 登录时一样，输入密码时屏幕不会显示任何字符。新密码不要使用学号、生日、姓名、简单数字，也不要复用其他网站或学校账号的密码。

如果修改失败，或者你已经忘记当前密码，请联系 Hirakiuchi-san 或服务器管理员处理。
