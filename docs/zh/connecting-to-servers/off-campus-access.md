...
如果你在家里、宿舍、咖啡店、出差地点，或者使用手机热点，一般不能直接访问实验室服务器。VIS Lab 的服务器属于实验室内的共享研究资源，校外访问方式必须符合学校和实验室的安全要求。
...
| --- | --- | --- |
| 实验室内 Zone C  WI-FI 网络 | :white_check_mark: :white_check_mark: | 直接按 [SSH 是什么以及如何连接](ssh-keys.md) 操作。 |
